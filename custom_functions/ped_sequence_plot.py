...
def ind_seq_dict(data, video, frame, idx, mode ='video_file'):
    """
    Note we use the first frame and id and the search parameters
    This can sort dict directly. Note this is not compute
    idx=idy
    """
    # Make sure of shape of vid_loc, frame_loc, id_y is shape of traj
    if mode == 'video_file':
        vid_loc = data['video_file'].reshape(-1,1) #videos locations
    elif mode == 'vid_name':
        vid_loc = data[mode].reshape(-1,1) #videos locations
        
    frame_loc = data['frame_y'][:,0].reshape(-1,1) # frame locations
    id_y = data['id_y'][:,0].reshape(-1,1)
    vid_frame_id_y = []
    ...
    if mode == 'video_file':
        ...
        find = '{}'.format(video) + '_' + '{}'.format(frame) + '_' + '{}'.format(idx)
    for jj in vid_frame_id_y:
        if jj == find:
            found_index = i 
            break
        else:
