...
def model1(x_train,y_train, x_test,y_test):
    ...
    keras.layers.Dense({{choice([50,100])}}),
    keras.layers.Dense(4)
    ])
    opt = tf.keras.optimizers.Adam(learning_rate={{uniform(1e-7,1e-5)}})
    model.compile(loss=bb_intersection_over_union, optimizer=opt, metrics=['accuracy'])
    result = model.fit(x_train,y_train,
                      batch_size= 32,
                      epochs =2,
                      validation_split=0.1)
    validat_acc = np.amax(result.history['val_accuracy'])
    print('Best validation acc of epoch:', validat_acc)
    return {'loss': -validat_acc, 'status': STATUS_OK, 'model': model}
